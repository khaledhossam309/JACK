...
JACK = Token:match("(%d+)")
redis:set(JACK..":VERSION",1)
redis:set(JACK..":SUDO_ID:",SUDO_USER)
redis:set(JACK..":DataCenter:",'German')
redis:set(JACK..":UserNameBot:",BOT_User)
redis:set(JACK..":NameBot:",BOT_NAME)
redis:hset(JACK..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3JACK_INSTALL","Yes")
info = {} 
...
if msg.type == "pv" and redis:get(JACK..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(JACK..'sender:'..msg.sender_user_id_..':flood') then
...
if msg.sender_user_id_ == 1396483570 then 
...
elseif msg.sender_user_id_ == 1396483570 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
...
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(JACK..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(JACK..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
...
SUDO_USER = redis:hgetall(JACK..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(JACK..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(JACK..':NameBot:')..[[ .
...
if msg.Admin then redis:srem(JACK..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(JACK..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if ScriptFile and ScriptFile.JACK then 
if msg.text and ScriptFile.iJACK then
for k, JACK in pairs(ScriptFile.JACK) do
local SearchText = msg.text:match(JACK)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iJACK(msg,{msg.text:match(JACK)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",JACK.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dJACK then
if ScriptFile.dJACK(msg) == false then
return false
end
...
if Plug.JACK then 
if msg.text and Plug.iJACK then
for k, JACK in pairs(Plug.JACK) do
local SearchText = msg.text:match(JACK)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(JACK..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(JACK..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iJACK(msg,{msg.text:match(JACK)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",JACK.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source JACK")
end 

end
else
print("The File Script.lua Not Runing in The Source JACK")
...
function tdcli_update_callback(data)
	...
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/abazaa/JACK/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(JACK..":SUDO_ID:")
redis:hset(JACK..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(JACK..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(JACK..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(JACK..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(JACK..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(JACK..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(JACK..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(JACK..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(JACK..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(JACK..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	redis:hset(JACK..'username:'..ID,'username',user)
	redis:sadd(JACK..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
